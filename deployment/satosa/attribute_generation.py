...
you don't care which value is used in a template.
    ...
    def _synthesize(self, attributes, requester, provider):
        ...
        mail = context.get("mail", False)
        if not mail:
            context["mail"] = MustachAttrValue("mail", [self.get_fake_mail(attributes)])

        schac_home_organisation = context.get("schac_home_organisation", False)
        if not schac_home_organisation:
            context["schac_home_organisation"] = MustachAttrValue("schac_home_organisation", [self.get_fake_schac_home_organisation(attributes)])

        # Beeld en Geluid wants IdP in an attribute
        context["idp"] = MustachAttrValue("idp", [provider])
    ...
    def get_fake_mail(self, attributes):
        for attr_name,values in attributes.items():
            if attr_name == "eppn":
                return values[0]

    def get_fake_schac_home_organisation(self, attributes):
        for attr_name,values in attributes.items():
            if attr_name == "eppn":
